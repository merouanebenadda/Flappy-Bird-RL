    ...
    for _ in range(hyperparams["num_episodes"]):
        ...
        while True:
            ...
            if len(replay_buffer.buffer) > hyperparams["batch_size"]:
                ...
                states = torch.as_tensor([exp.state for exp in batch], dtype=torch.float32).to(device)
                actions = torch.as_tensor([exp.action for exp in batch], dtype=torch.int64).unsqueeze(1).to(device)
                rewards = torch.as_tensor([exp.reward for exp in batch], dtype=torch.float32).to(device)
                next_states = torch.as_tensor([exp.next_state for exp in batch], dtype=torch.float32).to(device)
                dones = torch.as_tensor([exp.done for exp in batch], dtype=torch.float32).to(device)   
